    ...
    >
        ...
        >
            ...
            <div class="login-info">
                ...
                <a
                    class="form__link"
                    href="https://xiaoshen.blog.csdn.net/article/details/131933687"
                    target="_blank"
                    >{{ $t('GetToken') }}</a
                >
                &nbsp;&nbsp;&nbsp;&nbsp;
...
const loginAction = async () => {
    ...
    if (
        ...
        if (loginRes.status === 200) {
            const rsaContent = JSON.parse(
                rsaDecode(atob((loginRes.data as any).content))
            )
            // // console.log("rsaDecode----", rsaContent, loginForm);
            if (
                rsaContent.userName === loginForm.userName &&
                rsaContent.passWord === loginForm.passWord
            ) {
...
const registUser = async () => {
    ...
    if (loginForm.userName && loginForm.passWord && loginForm.gitToken) {
        ...
        if (res.status === 200) {
            ...
            const registRes = await loginApi.registUser(
                loginForm.gitToken,
                userInfo
            )
